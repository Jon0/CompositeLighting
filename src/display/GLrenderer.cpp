...
GLrenderer::GLrenderer(GLuint addr, GLuint w, GLuint h): buff(GL_ARRAY_BUFFER, false) {
    pbo_addr = addr;
	width = w, height = h;

    glGenTextures(1, &tex_addr);
	glBindTexture(GL_TEXTURE_RECTANGLE_NV, tex_addr); //GL_TEXTURE_RECTANGLE_NV
...
void GLrenderer::copyPbo(GLuint addr) {
    // currently only float4 rgba format is supported
