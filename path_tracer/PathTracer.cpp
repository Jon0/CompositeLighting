...
void PathTracerScene::initScene(InitialCameraData& camera_data) {
	...
	// setup mesh programs
	std::string mesh_ptx_path = ptxpath("path_tracer",
			"triangle_mesh_iterative.cu");
	Program bounding_box = m_context->createProgramFromPTXFile(mesh_ptx_path,
			"mesh_bounds");
	Program intersection = m_context->createProgramFromPTXFile(mesh_ptx_path,
			"mesh_intersect");
	scene.setMeshPrograms(bounding_box, intersection);

	// material programs
	diffuse_ch_out = m_context->createProgramFromPTXFile(
			ptxpath("path_tracer", "path_tracer.cu"), "diffuse_outline");
	diffuse_ch = m_context->createProgramFromPTXFile(
			ptxpath("path_tracer", "path_tracer.cu"), "diffuse");
	diffuse_ah = m_context->createProgramFromPTXFile(
			ptxpath("path_tracer", "path_tracer.cu"), "shadow");
	if (outline) {
